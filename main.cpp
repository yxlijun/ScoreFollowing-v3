...
int main()
{
 ...
 //   
	...
	const string scoreEventPath = "../Data/Pad/11.14/TangLang/TangLang.dat";
	ScoreFollowing scoreFollowing;
	if (scoreFollowing.Init(scoreEventPath) == -1) {
		cout << "Read scoreEvent error!" << endl;
		exit(-1);
	...
	int maxPitchesInEvent = scoreFollowing.GetMaxPitchesInFrame();
	vector<vector<double> > H;
	if (ReadH("../Data/Pad/11.14/TangLang/xH88.txt", H) == -1) {
		cout << "Read H error!" << endl;
		exit(-1);
	}
	vector<double> error;
	...
	SaveSfResult("../Data/Pad/11.14/TangLang/sfResult.txt", scoreFollowing.GetSfResult());
	SaveSfResult("../Data/Pad/11.14/TangLang/sfResultOrigin.txt", scoreFollowing.GetSfResultOrigin());
	...
	int starsNum = evaluateResult.GiveStars(score);
	cout << "score = " << score << " stars = " << starsNum << endl;
	evaluateResult.SaveEvaluateResult("../Data/Pad/11.14/TangLang/evaluateResult.txt", correctness1, beatRhythm);

	vector<Correctness> correctnessOrigin = evaluateResult.EvaluateCorrectnessOrigin(maxPitchesInEvent);
	evaluateResult.SaveEvaluateResult("../Data/Pad/11.14/TangLang/evaluateResultOrigin.txt", correctnessOrigin, beatRhythm);
    cout << "test" << endl;
    return 0;
}
